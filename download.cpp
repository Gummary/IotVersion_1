...
void DownLoad::UpdateProcess(QString url, qint64 recesize, qint64 totalsize, int usedTime)
{
    ...
    {
        QStringList list;
        list << url <<  QString::number((qlonglong)totalsize) << QString::number((qlonglong)recesize)
             << speed(dSpeed) << timeFormat(qCeil(dLeftTime));

        QString msg = QString("0/3/")+list.join(";");
        qDebug() << msg;
        if(service)
...
void DownLoad::SetSocketService(SocketClass *service)
{
    this->service = service;
}


int DownLoad::CancelTask(QUrl url)
{
    HttpReply *reply = downloadSet.value(url.toString(), NULL);
    if(reply!=NULL)
    {
        reply->CancelDownLoad();
        return (0);
    }
